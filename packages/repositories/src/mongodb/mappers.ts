...
export function mapMongoIssue(doc: any): Issue {
  if (!doc) return null as any;

  let reporter: string | PopulatedReporter;
  ...
  return {
    ...
    votes: doc.votes ?? 0,
    voter_ids: (doc.voter_ids || []).map((v: any) => v.toString()),
...
export function mapMongoUserSession(doc: any): UserSession {
  if (!doc) return null as any;
  return {
    id: doc._id ? doc._id.toString() : '',
...
export function mapMongoNotification(doc: any): Notification {
  if (!doc) return null as any;
  return {
    id: doc._id ? doc._id.toString() : '',
    userId: doc.userId ? doc.userId.toString() : '',
