...
export async function citizenAssistantStream(req: Request, res: Response) {
  ...
  logger.info(`[AIController] Starting Citizen Assistant stream`, {
    requestId,
    userId,
    endpoint: '/api/ai/assistant/citizen',
  });
...
Keep your responses concise, helpful, and friendly. Do not use markdown headers, just simple text.`;
    ...
    logger.info(`[AIController] Finished Citizen Assistant stream`, {
      requestId,
      userId,
      durationMs: duration,
      success: true,
    ...
    const duration = Date.now() - startTime;
    logger.error(`[AIController] Citizen assistant stream error: ${err.message}`, {
      requestId,
      userId,
      durationMs: duration,
      success: false,
...
export async function copilotStream(req: Request, res: Response) {
  ...
  logger.info(`[AIController] Starting Municipality Copilot stream`, {
    requestId,
    userId,
    endpoint: '/api/ai/assistant/copilot',
    provider: 'groq',
  ...
  try {
    ...
    for await (const chunk of stream) {
      ...
      if (
        chunk.includes('Error executing tool') ||
        chunk.includes('I am currently running in mock mode')
      ) {
        toolsInvoked++;
      }
      res.write(`data: ${JSON.stringify(chunk)}\n\n`);
    ...
    const duration = Date.now() - startTime;
    logger.info(`[AIController] Finished Municipality Copilot stream`, {
      requestId,
      userId,
      durationMs: duration,
      success: true,
    ...
    const duration = Date.now() - startTime;
    logger.error(`[AIController] Copilot stream error: ${err.message}`, {
      requestId,
      userId,
      durationMs: duration,
      success: false,
